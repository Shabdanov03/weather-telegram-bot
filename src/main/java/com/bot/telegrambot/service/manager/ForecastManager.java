...
import com.bot.telegrambot.dto.Data;
import com.bot.telegrambot.dto.WeatherResponse;
import com.bot.telegrambot.service.WeatherService;
...
import static com.bot.telegrambot.data.QueryData.*;
...
public class ForecastManager {
    ...
    public BotApiMethod<?> answerCommand(Message message, Bot bot) {
        return SendMessage.builder()
                .text("""
                        ...
                        потом выберите вариант прогноза. ⬇️⬇️⬇️
                        """)
    ...
    public BotApiMethod<?> answerQuery(CallbackQuery query, String[] data, Bot bot) {
        ...
        switch (data[1]) {
            case "c" -> {
                return cityEntering(query, data.length == 2 ? "" : data[2]);
            }
            case "e" -> {
                if (data.length == 2) {
    ...
    private BotApiMethod<?> sendResult(CallbackQuery query, String type, String cityName, Bot bot) {
        ...
        for (Data data : weatherResponse.getData()) {
            sb.append("\uD83D\uDCCD +").append(number).append(")\n")
                    ...
                    .append("Скорость ветра: ").append(data.getWinSpd()).append("\n")
    ...
    private BotApiMethod<?> cityEntering(CallbackQuery query, String current) {
        return EditMessageText.builder()
                .text("Введите название города используя интерактивную клавиатуру")
                .replyMarkup(cityEnteringKeyboard(current))
                .messageId(query.getMessage().getMessageId())
                .chatId(query.getMessage().getChatId())
                .build();
    }
    ...
    private InlineKeyboardMarkup cityEnteringKeyboard(String current) {
        return keyboardFactory.getInlineKeyboardMarkup(
                List.of(
